# chatbot_wav2lip_subprocess.py - Press SPACE to talk, get video response with Wav2Lip
...
WAV2LIP_DIR = "C:\\Programming\\wav2lip-onnx-HQ"
WAV2LIP_VENV = "C:\\Programming\\wav2lip-onnx-HQ\\venv"
WAV2LIP_CHECKPOINT = "checkpoints/wav2lip_gan.onnx"
...
print("="*60)
print("🎬 INITIALIZING AI AVATAR CHATBOT (Wav2Lip Subprocess)")
print("="*60)
...
def generate_wav2lip_video(audio_path, output_video_name):
    """Generate video with Wav2Lip ONNX via subprocess"""
    ...
    try:
        ...
        # Build PowerShell command to run in Wav2Lip venv
        # We'll run a simple one-liner that activates venv and runs inference
        ps_command = f"""
        $env:PYTHONHASHSEED='random'
        cd '{WAV2LIP_DIR}'
        & '{WAV2LIP_VENV}\\Scripts\\python.exe' inference_onnxModel_auto.py --checkpoint_path {WAV2LIP_CHECKPOINT} --face '{abs_avatar_path}' --audio '{abs_audio_path}' --outfile temp_output.mp4 --fps 15
        """
        ...
        temp_output = os.path.join(WAV2LIP_DIR, "temp_output.mp4")
        if not os.path.exists(temp_output):
            # Try results folder
            temp_output = os.path.join(WAV2LIP_DIR, "results", "test_output.mp4")
        
        if os.path.exists(temp_output):
            # Copy to our outputs directory
            shutil.copy(temp_output, abs_output_path)
            ...
            print("❌ Wav2Lip output not found")
            return None
            
    except subprocess.TimeoutExpired:
        print(f"❌ Wav2Lip timeout after 120 seconds")
        return None
    except Exception as e:
...
def run_press_to_talk_chatbot():
    ...
    print(f"Lip-sync: Wav2Lip ONNX (subprocess)")
    print("\n📢 Controls:")
